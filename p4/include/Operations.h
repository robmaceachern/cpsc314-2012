...
public:
	...
	Vec3 scale(double factor) {
		return Vec3(coord[0] * factor,
					coord[1] * factor,
					coord[2] * factor);
	}

	// returns a new vector representing the substraction of this vector with other
	Vec3 subtract(Vec3 other) {
		return Vec3(coord[0] - other[0],
					coord[1] - other[1],
					coord[2] - other[2]);
	}

	// returns a new vector representing the addition of this vector with other
	Vec3 add(Vec3 other) {
		return Vec3(coord[0] + other[0],
					coord[1] + other[1],
					coord[2] + other[2]);
