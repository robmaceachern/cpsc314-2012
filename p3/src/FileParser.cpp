					...
					Camera *camera, int *resolution)
{
	...
	while(in.getline(currentLine,256)) {
		...
		// material
			...
			// read material id
			istringstream(line.substr(8,line.size())) >> material.id;
		...
		// sphere
			...
			istringstream(line.substr(6,line.size())) >> sphere.radius >> sphere.center[0] >> sphere.center[1] >> sphere.center[2] >> sphere.materialID;
		...
		// plane
			...
			istringstream(line.substr(5,line.size())) >> plane.params[0] >> plane.params[1] >> plane.params[2] >> plane.params[3] >> plane.materialID;
